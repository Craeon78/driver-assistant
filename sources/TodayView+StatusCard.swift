    //======================================
    // MARK: - TodayView+StatusCard
    //======================================
    //
    // Purpose:
    // - Quick-glance shift status card for the Today view.
    //
    // Displays:
    // - Driver/truck/odo summary
    // - Current shift and segment kilometre estimates
    // - Raw GPS evidence (diagnostic only)
    // - Work/rest/drive totals
    // - Start Shift / End Shift actions
    //
    // Notes:
    // - "Km this shift" and "Km this segment" come from the AppModel
    //   segment-based distance model (finalised km + pending corrected GPS).
    // - Raw GPS distance is shown separately for comparison/debug and is
    //   not authoritative.
    // - ODO remains the authoritative anchor for distance truth.
    //
    ...
    var statusCard: some View {
        ...
        VStack(alignment: .leading, spacing: 8) {
            ...
            if !model.isOnDuty {
                ...
                // optional debug line:
                Text("GPS evidence only: \(kmDisplay(liveKm)) km")
                    .font(.caption2)
