...
//
// Path: AppModels/AppModel+ShiftActions.swift
//
// Purpose:
// - Central activity engine for shift lifecycle, activity transitions,
//   and real-time state updates.
//
// Responsibilities:
// - Gate all activity button presses (drive, break, load, unload, end shift).
// - Manage segment lifecycle (start, close, transition).
// - Maintain authoritative shift state (on duty, driving, break).
// - Handle odometer capture gating for compliance-critical transitions.
// - Record events and trigger autosave checkpoints.
// - Provide live time accumulation (tick) for driving.
//
// Boundaries:
// - Does NOT perform fatigue rule calculations (see RestLogic / FatigueRules).
// - Does NOT handle persistence (autosave only triggers here).
// - Does NOT own UI — only exposes state and actions.
//
// State Model Notes:
// - `currentActivity`, `currentSegmentStart`, `segmentsToday` are the source of truth.
// - `isDriving`, `isOnBreak`, `isOnDuty` are UI + behaviour flags.
// - If divergence occurs, activity engine state takes precedence.
//
// Notes:
// - Pre-persistence: operates on in-memory "today" data only.
// - Post-persistence: will become the orchestration layer over stored timelines.
// - ODO capture gates are critical to maintaining compliance-grade auditability.
//
// Phase: Pre-persistence → Transitional (becoming persistence-aware)
//======================================
