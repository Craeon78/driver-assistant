...
public enum Chunk5CargoFoundationGateTests {
    public static func run() -> [String] {
        var results: [String] = ["=== V3 Chunk 5A Generic Cargo Foundation Gate v2 ==="]
        ...
        check("Duplicate compartment limits rejected before projection", fails { _ = try CargoLedger(limits: [CargoCompartmentLimit(compartmentID: c1, capacityUnits: 100), CargoCompartmentLimit(compartmentID: c1, capacityUnits: 200)]) })
        ...
        check("Duplicate transaction ID rejected", fails {
            var l = try CargoLedger(limits: limits)
            let tx = CargoTransaction(kind: .load, cargo: cargo, units: 100, destinationCompartmentID: c1, occurredAt: t0)
        ...
        check("Correction rejects descriptor drift", fails {
            ...
            try l.append(load)
            var altered = cargo
            altered.kilogramsPerUnit = 99
            try l.append(CargoTransaction(kind: .correction, cargo: altered, units: 100, sourceCompartmentID: c1, occurredAt: t0.addingTimeInterval(1), correctsTransactionID: load.id))
        })
        ...
        check("Decode rejects malformed transaction endpoints", succeeds {
            let malformed = CargoTransaction(kind: .load, cargo: cargo, units: 50, occurredAt: t0)
            struct Envelope: Encodable { let limits: [CargoCompartmentLimit]; let transactions: [CargoTransaction] }
            let data = try JSONEncoder().encode(Envelope(limits: limits, transactions: [malformed]))
            do { _ = try JSONDecoder().decode(CargoLedger.self, from: data); return false } catch { return true }
        })

        check("Decode rejects duplicate transaction IDs", succeeds {
            let tx = CargoTransaction(kind: .load, cargo: cargo, units: 50, destinationCompartmentID: c1, occurredAt: t0)
            struct Envelope: Encodable { let limits: [CargoCompartmentLimit]; let transactions: [CargoTransaction] }
            let data = try JSONEncoder().encode(Envelope(limits: limits, transactions: [tx, tx]))
            do { _ = try JSONDecoder().decode(CargoLedger.self, from: data); return false } catch { return true }
        })
